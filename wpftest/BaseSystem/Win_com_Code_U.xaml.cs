                ...
                if (ExpExc.DialogResult.HasValue)
                {
                    if (ExpExc.choice.Equals(dgdMcg.Name))
                    {
                        ...
                        if (ExpExc.Check.Equals("Y"))
                            dt = Lib.Instance.DataGridToDTinHidden(dgdMcg);
                        else
                            dt = Lib.Instance.DataGirdToDataTable(dgdMcg);
                    ...
                    else if (ExpExc.choice.Equals(dgdScg.Name))
                    {
                        //MessageBox.Show("소분류");
                        if (ExpExc.Check.Equals("Y"))
                            dt = Lib.Instance.DataGridToDTinHidden(dgdScg);
                        else
                            dt = Lib.Instance.DataGirdToDataTable(dgdScg);
                        Name = dgdScg.Name;
            ...
            try
            {
                if (CheckData())
                {
                    ...
                    if (winCode != null)
                    {
                        ...
                        //테스트용 sql문
                        //string strConn = "data source=wizis.iptime.org,20140;Initial catalog=MES_HanMin; Integrated Security=True; connection timeout=180";
