    ...
    public partial class Win_com_Custom_U : UserControl
    {
                    ...
                    else
                    {
                        ...
                        foreach (DataRow dr in drc)
                        {
                            ...
                            var WinCustom = new Win_com_Custom_U_CodeView()
                            {
                                ...
                                CustomNo = CustomFormat(dr["CustomNo"].ToString()),
                            ...
                            tbkSearchIndex.Text = "검색건수 : " + i + " 건"; //2021-09-24 정현달 합계 추가

                            //txtCustomNO.Text = CustomFormat(WinCustom.CustomNo());
                            //txtCustomNO.Text = WinCustom.CustomNo.Substring(0, 3);
                            //CustomNo
        ...
        private bool SaveData(string strCustomID, string strFlag)
        {
            ...
            try
            {
                if (CheckData())
                {
                    ...
                    sqlParameter.Add("sCustomNO", InsertCustomFormat(txtCustomNO.Text.ToString()));
        ...
        private string CustomFormat(string str)
        {
            ...
            if (result.Length == 10)
            {
                ...
                {
                    result = result.Substring(0, 3) + "-" + result.Substring(3, 2) + "-" + result.Substring(5, 5);
                }
            }
        ...
        // 사업자명 하이픈빼기
        private string InsertCustomFormat(string str)
        {
            string result = str.Replace("-", "");
        ...
        private void TxtCustomNO_TextChanged(object sender, TextChangedEventArgs e)
        {
            TextBox txtSender = sender as TextBox;
            if (txtSender == null) return;

            txtSender.Text = CustomFormat(txtSender.Text);
        }
    }
